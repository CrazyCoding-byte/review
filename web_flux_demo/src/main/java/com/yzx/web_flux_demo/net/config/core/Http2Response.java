...
public class Http2Response implements Response {
    ...
    public void writeAndFlush() {
        ...
        try {
            ...
            io.netty.handler.codec.http2.DefaultHttp2HeadersFrame headersFrame =
                    new io.netty.handler.codec.http2.DefaultHttp2HeadersFrame(nettyHeaders, false); // endStream = false
            streamChannel.writeAndFlush(headersFrame);

            // 写入 body 帧 (endStream = true)
            ByteBuf bodyBuffer = streamChannel.alloc().buffer().writeBytes(body);
            io.netty.handler.codec.http2.DefaultHttp2DataFrame dataFrame =
                    new io.netty.handler.codec.http2.DefaultHttp2DataFrame(bodyBuffer, true); // endStream = true
